...
struct FloatingActionBarView: View {
    ...
    private func extractDateFromMessage(_ message: String) -> Date? {
        ...
        // Handle relative offsets like "in 2 hours" or "in 30 minutes"
        if let relativeMatch = try? NSRegularExpression(pattern: "in\\s+(\\d+)\\s+(hour|hours|hr|hrs)", options: .caseInsensitive),
           let match = relativeMatch.firstMatch(in: message, range: NSRange(location: 0, length: message.count)),
        ...
        if let minuteMatch = try? NSRegularExpression(pattern: "in\\s+(\\d+)\\s+(minute|minutes|min|mins)", options: .caseInsensitive),
           let match = minuteMatch.firstMatch(in: message, range: NSRange(location: 0, length: message.count)),
           let valueRange = Range(match.range(at: 1), in: message),
           let value = Int(message[valueRange]) {
        ...
        if let explicitDate = parseExplicitDate(from: lowercased, calendar: calendar, now: now) {
        ...
        var inferredTime: (hour: Int, minute: Int)? = parseExplicitTime(from: message)
